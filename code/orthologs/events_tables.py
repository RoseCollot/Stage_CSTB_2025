...
def table (path_to_scpecies_table, path_to_new_species_df):
    ...
    for gene in df['gene'].unique():
        ...
        nb_duplication = ((df_gene['event'] == 'duplication') & (df_gene['query_coverage'] >= 80)).sum()
        ...
        row.append(sum([nb_other,nb_fusion,nb_fission,nb_duplication]))
        row.append(int(nb_whole))
        row.append(int(nb_fusion))
        row.append(int(nb_fission))
        row.append(int(nb_duplication))
        row.append(int(nb_other))
        row_df.append(row)
    gene_ali_df = pd.DataFrame(row_df, columns= ['gene', 'species', 'number_of_exons','number_of_hits','number_of_events','no_event', 'fusion', 'fission', 'duplication', 'others'])      
