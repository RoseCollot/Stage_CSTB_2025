...
def create_ks_df (table_species, path_to_genomes_df_dir,data_column, pvalue_or_statistic ) : 
    ...
    genid_species = dict(zip(df_table['genome_id'], df_table['species_name']))
    genomes_list = []
    species_list = []
    df=[]
    ...
    for i in range(len(genomes_list)): 
        path_dir= os.path.join(path_to_genomes_df_dir,genomes_list[i])
    ...
    df_ks = df_ks.reindex(index=species_order, columns=species_order)
    return df_ks
